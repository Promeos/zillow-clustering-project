...
def cluster_features(train, validate, test):   
    cluster_columns_1 = ['longitude',
                         'latitude',
                         'structure_tax']

    cluster_columns_2 = ['has_basement',
                         'has_hottub_or_spa',
                         'has_pool',
                         'has_patio',
                         'has_shed']

    cluster_columns_3 = ['living_room_area_sqft',
                         'lot_size_sqft',
                         'year_built']
    ...
    cluster_1 = train[cluster_columns_1].copy()
    cluster_2 = train[cluster_columns_2].copy()
    cluster_3 = train[cluster_columns_3].copy()

    train['cluster_1'] = kmeans1.fit(cluster_1).labels_
    validate['cluster_1'] = kmeans1.predict(validate[cluster_columns_1])
    test['cluster_1'] = kmeans1.predict(test[cluster_columns_1])

    train['cluster_2'] = kmeans2.fit(cluster_2).labels_
    validate['cluster_2'] = kmeans2.predict(validate[cluster_columns_2])
    test['cluster_2'] = kmeans2.predict(test[cluster_columns_2])

    train['cluster_3'] = kmeans3.fit(cluster_3).labels_
    validate['cluster_3'] = kmeans3.predict(validate[cluster_columns_3])
    test['cluster_3'] = kmeans3.predict(test[cluster_columns_3])
